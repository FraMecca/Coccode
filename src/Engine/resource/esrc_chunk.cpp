...
std::pair<ChunkMemState, Chunk*> get_chunkPtr(chunkKey_t key_)
{
    ...
    return (state == ChunkMemState::Active) ? std::pair<ChunkMemState, Chunk*>{ state, chunk_inn::chunks.at(key_).get() } : std::pair<ChunkMemState, Chunk*>{ state, nullptr }; // let caller handle the exception
}
