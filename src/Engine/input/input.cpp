...
void processInput(GLFWwindow* windowPtr_)
{
    ...
    if (isFind_joyLeftAxes) {
        ...
        if (kbRef.get_isAnyKeyPress()) {
            // 检查键盘 WSAD
            if (kbRef.get_key(KeyBoard::Key::W)) {
                joyLeftAxes.y = 1.0;
                is_find_dirKeys = true;
            }
            if (kbRef.get_key(KeyBoard::Key::S)) {
                joyLeftAxes.y = -1.0;
                is_find_dirKeys = true;
            }
            if (kbRef.get_key(KeyBoard::Key::A)) {
                joyLeftAxes.x = -1.0;
                is_find_dirKeys = true;
            }
            if (kbRef.get_key(KeyBoard::Key::D)) {
                joyLeftAxes.x = 1.0;
                is_find_dirKeys = true;
            }
        }
...
namespace input_inn { //------------------- namespace: input_inn ----------------------//
    ...
    void debug_inputData()
    {
        ...
        for (const auto& [joyIdx, joyUPtr] : input_inn::inputDataUPtr->joysticks) {
            ...
            if (joyRef.get_button(Joystick::Button::LB)) {
                tprDebug::console("LB, ");
            }
            if (joyRef.get_button(Joystick::Button::RB)) {
                tprDebug::console("RB, ");
            }
            if (joyRef.get_button(Joystick::Button::Back)) {
                tprDebug::console("Back, ");
            }
            if (joyRef.get_button(Joystick::Button::Start)) {
                tprDebug::console("Start, ");
            }
            if (joyRef.get_button(Joystick::Button::Guide)) {
                tprDebug::console("Guide, ");
            }
            if (joyRef.get_button(Joystick::Button::LeftThumb)) {
                tprDebug::console("LeftThumb, ");
            }
            if (joyRef.get_button(Joystick::Button::RightThumb)) {
                tprDebug::console("RightThumb, ");
            }
            if (joyRef.get_button(Joystick::Button::Hat_Up)) {
                tprDebug::console("Hat_Up, ");
            }
            if (joyRef.get_button(Joystick::Button::Hat_Down)) {
                tprDebug::console("Hat_Down, ");
            }
            if (joyRef.get_button(Joystick::Button::Hat_Left)) {
                tprDebug::console("Hat_Left, ");
            }
            if (joyRef.get_button(Joystick::Button::Hat_Right)) {
                tprDebug::console("Hat_Right, ");
            }
        }
    }
