...
void DirAxes::limit_vals(const glm::dvec2 val_) noexcept
{
    ...
    (std::abs(normal.x) < std::abs(val_.x)) ? this->originVal.x = normal.x : this->originVal.x = val_.x;

    (std::abs(normal.y) < std::abs(val_.y)) ? this->originVal.y = normal.y : this->originVal.y = val_.y;
