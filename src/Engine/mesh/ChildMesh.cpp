...
void ChildMesh::refresh_translate()
{
    ...
    glm::dvec2 outDPos = isNeedCoordTransform ? worldCoord.calc_outDPos(currentDPos) : currentDPos;
    ...
    if (this->isPic == true) {
        ...
        if (goMeshRef.isPicFixedZOff) {
            this->translate_val.z = static_cast<float>(esrc::get_camera().get_zFar() + goMeshRef.picBaseZOff + zOff);
        } else {
