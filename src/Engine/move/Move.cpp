...
void Move::renderUpdate_crawl()
{
    ...
    //----------------//
    glm::dvec2 speedVec = this->crawlDirAxes.get_newVal().get_worldVal() * SpeedLevel_2_val(this->moveSpeedLvl.get_newVal()) * 60.0 * esrc::get_timer().get_smoothDeltaTime();
...
void Move::renderUpdate_drag()
{
    ...
    //- 等效于 DirAxes 的计算。
    glm::dvec2 speedVec = glm::normalize(dposOff) * SpeedLevel_2_val(this->moveSpeedLvl.get_newVal()) * 60.0 * esrc::get_timer().get_smoothDeltaTime();

    bool isLastFrame = false;
    if ((std::abs(speedVec.x) > std::abs(dposOff.x)) || (std::abs(speedVec.y) > std::abs(dposOff.y))) {
        isLastFrame = true;
        speedVec = dposOff;
    }
