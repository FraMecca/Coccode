    ...
    for (const auto& fieldKey : fieldKeys) { //- each field key
        ...
        for (int eh = 0; eh < ENTS_PER_FIELD<>; eh++) {
            for (int ew = 0; ew < ENTS_PER_FIELD<>; ew++) { //- each ent in field
                ...
                if (mapAlti < minFieldAlti) {
                    minFieldAlti = mapAlti;
                }
                if (mapAlti > maxFieldAlti) {
                    maxFieldAlti = mapAlti;
                }
                //--- skip unborder ent ---//
    ...
    Job_MapEnt& mapEnt_)
{
    ...
    case NineDirection::LeftBottom:
        ecoReadOnlyPtr = container_.at(0).get();
        break;
    case NineDirection::RightBottom:
        ecoReadOnlyPtr = container_.at(1).get();
        break;
    case NineDirection::LeftTop:
        ecoReadOnlyPtr = container_.at(2).get();
        break;
    case NineDirection::RightTop:
        ecoReadOnlyPtr = container_.at(3).get();
        break;
    default:
        tprAssert(0);
        break;
