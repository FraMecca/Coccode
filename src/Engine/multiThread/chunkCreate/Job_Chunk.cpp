...
void Job_Chunk::init() noexcept
{
    ...
    for (size_t h = 0; h < FIELDS_PER_CHUNK<size_t>; h++) {
        for (size_t w = 0; w < FIELDS_PER_CHUNK<size_t>; w++) {
            tmpFieldMPos = this->chunkMPos + IntVec2{ static_cast<int>(w) * ENTS_PER_FIELD<>, static_cast<int>(h) * ENTS_PER_FIELD<> };
...
bool Job_Chunk::is_mapEnt_a_ecoBorder(IntVec2 mposOff_) noexcept
{
    ...
    tprAssert((mposOff_.x >= 0) && (mposOff_.x < ENTS_PER_CHUNK<>)&&(mposOff_.y >= 0) && (mposOff_.y < ENTS_PER_CHUNK<>)); //- [0,31]
    ...
    for (const auto& mpOff : *nearMPsPtr) {
        ...
        tprAssert((tmpOffV.x >= 0) && (tmpOffV.x < ENTS_PER_CHUNK<>)&&(tmpOffV.y >= 0) && (tmpOffV.y < ENTS_PER_CHUNK<>));
        //-- do check --
