...
void EcoObjBorder::init()
{
    ...
    tprAssert(pixNum_per_frame.x == (ENTS_PER_SECTION<> + 1) && pixNum_per_frame.y == (ENTS_PER_SECTION<> + 1));
    ...
    for (const auto& frameData : frame_data_ary) {
        ...
        for (int j = 0; j < ENTS_PER_SECTION<>; j++) {
            for (int i = 0; i < ENTS_PER_SECTION<>; i++) { // eac pix
                ...
                if (pix.is_near(leftBottomColor, 5)) {
                    pixDir = NineDirection::LeftBottom;
                } else if (pix.is_near(rightBottomColor, 5)) {
                    pixDir = NineDirection::RightBottom;
                } else if (pix.is_near(leftTopColor, 5)) {
                    pixDir = NineDirection::LeftTop;
                } else if (pix.is_near(rightTopColor, 5)) {
                    pixDir = NineDirection::RightTop;
                } else {
