...
void signUp_newGO_to_chunk_and_mapEnt(GameObj& goRef_)
{
    ...
    if ((goRef_.family != GameObjFamily::Major) && (goRef_.family != GameObjFamily::BioSoup)) {
        return;
    }
    ...
    if (colliType == ColliderType::Circular) {
        ...
        for (const auto& mpos : currentSignINMapEntsRef) {
            ...
            if ((chunkState == ChunkMemState::NotExist) || (chunkState == ChunkMemState::WaitForCreate) || (chunkState == ChunkMemState::OnCreating)) {
                continue;
            }
