...
size_t calc_go_uWeight(IntVec2 mpos_, goSpeciesId_t goSpeciesId_, goLabelId_t goLabelId_, size_t extra_) noexcept
{
    ...
    size_t u = mapEntUWeight ^ (static_cast<size_t>(goSpeciesId_) << 1) ^ (static_cast<size_t>(goLabelId_) << 2);
    ...
    size_t ExtraGoUWeight_)
{
    ...
    for (const auto& [jgomeshName, jgomesh] : planRef.gomeshs) { // each json goMesh
        ...
        (jgomesh.isAutoInit) ? goDUPtr->goMeshs_autoInit.push_back(std::move(gmSPtr)) : goDUPtr->goMeshs_notAutoInit.push_back(std::move(gmSPtr)); // move
    }
    //===
    return goDUPtr;
}
